...
func (e *Editor) KeyEnter() {
	...
	for e.it_char != e.it_line.Value.End() {
		item := e.it_line.Value.Erase(e.it_char)
		nova.Insert(nova.End(), item.Value)
	}
...
func (e *Editor) KeyUp() {
	if e.it_line != e.texto.Front() {
		...
		e.it_char = e.it_line.Value.Front()
		for i := 0; i < pos && e.it_char != e.it_line.Value.End(); i++ {
			e.it_char = e.it_char.Next()
		}
	}
}
...
func (e *Editor) KeyBackspace() {
	if e.it_char.Prev() != e.it_line.Value.End() {
		e.it_char = e.it_line.Value.Erase(e.it_char.Prev()) // pra remover o caractere a esquerda do cursor
	} else if e.it_line != e.texto.Front() {
		prevLine := e.it_line.Prev()
		alvoChar := prevLine.Value.End()

		pos_atual := e.it_line.Value.Front()
		for pos_atual != e.it_line.Value.End() {
			prevLine.Value.Insert(prevLine.Value.End(), pos_atual.Value)
			pos_atual = pos_atual.Next()
		}

		linha_anterior := e.it_line
		e.it_line = prevLine
		e.it_char = alvoChar
		e.texto.Erase(linha_anterior)
	}
...
func (e *Editor) KeyDelete() {
	if e.it_char != e.it_line.Value.End() {
		e.it_char = e.it_line.Value.Erase(e.it_char) // pra remover o caractere acima do cursor
	} else if e.it_line.Next() != e.texto.End() {
		nextLine := e.it_line.Next()

		pos_atual := nextLine.Value.Front()
		for pos_atual != nextLine.Value.End() {
			e.it_line.Value.Insert(e.it_line.Value.End(), pos_atual.Value)
			pos_atual = pos_atual.Next()
		}
